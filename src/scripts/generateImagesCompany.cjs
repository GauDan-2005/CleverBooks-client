...
const imagesDir = path.join(__dirname, "..", "assets", "companies");

// Output file
const outputFile = path.join(__dirname, "..", "constants", "companyImages.js");
...
fs.readdir(imagesDir, (err, files) => {
  ...
  const imports = imageFiles
    .map((file) => {
      ...
      return `import ${variableName} from "../assets/companies/${file}";`;
    })
    .join("\n");
